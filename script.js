...
//Variable
const blockArr = [];
const numBlockCol = 11;
const numBlockRow = 5;
const colWidth = WIDTH / numBlockCol;
const padding = colWidth / 4;
let blockIndex = 0;
let isGameOver = false;
let gameLevel = 1;
let numObBlocksHit = 0;
let isLeft = false;
let isRight = false;
...
//block
class Block {
  constructor(x, y, width, height, isShow) {
    this.x = x;
    this.y = y;
    this.width = width;
    this.height = height;
...
Ball.prototype.updateBall = function (playerObj) {
  ...
  if (
    this.x + this.radius > playerObj.x &&
    this.x - this.radius < playerObj.x + playerObj.width &&
  ...
  if (
    ...
    if (playerObj.lifeCount <= 0) {
      ...
      this.x = WIDTH / 2;
      this.y = HEIGHT / 2;
      this.xDirSpeed = Math.random() < 0.5 ? 1.5 : -1.5;
      this.yDirSpeed = Math.random() < 0.5 ? 1.5 : -2;
    }
  }
};
...
const ball = new Ball(ballProp.x, ballProp.y, ballProp.radius);
  ...
  for (let i = 1; i < numBlockCol; i++) {
    for (let j = 0; j < numBlockRow; j++) {
      blockArr[blockIndex] = new Block(
        ...
        0.7 * colWidth + j * (colWidth - 2 * padding),
        blockProp.w - padding,
        blockProp.h,
        true
      );
  ...
  blockArr.forEach((b) => {
    if (isBallhitsBlock(ball, b) && b.isShow) {
      ...
      if (numObBlocksHit == (numBlockCol - 1) * numBlockRow) {
        ...
        document.getElementById('gameover').innerHTML = `Game Level 
          ${gameLevel}
           is completed. Click Next to begin next level`;
      }
    }
    if (b.isShow) {
      context.beginPath();
